...
MonthlyCostsState::MonthlyCostsState(Base *base) : _base(base)
{
	...
	int countryFunding = CountrySystem::getCountriesMonthlyFundingTotal();
	_txtIncome->setText(std::format("{}={}", tr("STR_INCOME"), Unicode::formatFunding(countryFunding)));

	int baseFunding = BaseSystem::getBasesMaintenanceCost();
	_txtMaintenance->setText(std::format("{}={}", tr("STR_MAINTENANCE"), Unicode::formatFunding(baseFunding)));
