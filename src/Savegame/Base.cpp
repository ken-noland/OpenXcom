...
{
	const entt::registry& registry = save->getRegistry();
	entt::entity countryId = AreaSystem::locate<Country>(registry, _lon, _lat);
	if (countryId != entt::null)
	{
		const Country& country = registry.get<Country>(countryId);
		_provideBaseFunc |= country.getRules()->getProvidedBaseFunc();
		_forbiddenBaseFunc |= country.getRules()->getForbiddenBaseFunc();
	}
	entt::entity regionId = AreaSystem::locate<Region>(registry, _lon, _lat);
	if (regionId != entt::null)
	{
		const Region& region = registry.get<Region>(regionId);
