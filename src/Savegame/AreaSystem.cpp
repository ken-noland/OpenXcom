...
{
	auto containsPoint = [lon, lat](const auto& entity) { return areaView.get<AreaType>(entity).getRules()->insideRegion(lon, lat); };
	return getRegistry().find_if(containsPoint);
}

template<typename AreaType>
static const AreaType* locateValue(double lon, double lat)
{
	auto containsPoint = [lon, lat](const AreaType& entity) { return areaView.get<const AreaType>(entity).getRules()->insideRegion(lon, lat); };
	return getRegistry().raw().try_get<AreaType>(getRegistry().find_if(containsPoint));
}
...
static bool addActivity(int amount, double lon, double lat)
{
	...
	(registry.get<AreaType>(areaId).*Getter)().back() += amount;
	return true;
}
