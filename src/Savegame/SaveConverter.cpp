...
namespace OpenXcom
{
	 ...
	 */
	SaveConverter::SaveConverter(int save, Mod* mod) : _mod(mod), _rules(mod->getConverter())
	{
	...
	void SaveConverter::loadDatIGlob()
	{
		...
		// Fix up the months
		size_t monthsPassed = static_cast<size_t>(month + (_year - _mod->getStartingTime().getYear()) * 12);
		...
		auto countryGetter = [](const std::string& countryName) { return getRegistry().findValueByName<Country>(countryName); };
		for (Country* country : _rules->getCountries() | std::views::transform(countryGetter))
		{
		...
		auto regionGetter = [](const std::string& regionName) { return getRegistry().findValueByName<Region>(regionName); };
		for (Region* region : _rules->getRegions() | std::views::transform(regionGetter))
		{
	...
	void SaveConverter::loadDatXcom()
	{
		...
		for (size_t i = 0; i < ENTRIES * MONTHS; ++i)
		{
			...
			{
				getRegistry().findValueByName<Country>(_rules->getCountries()[j])->getActivityXcom().push_back(score);
			}
			// region
			else
			{
				j -= _rules->getCountries().size();
				getRegistry().findValueByName<Region>(_rules->getRegions()[j])->getActivityXcom().push_back(score);
			}
		}
	}
	...
	void SaveConverter::loadDatAlien()
	{
		...
		for (size_t i = 0; i < ENTRIES * MONTHS; ++i)
		{
			...
			if (j < _rules->getCountries().size())
			{
				getRegistry().findValueByName<Country>(_rules->getCountries()[j])->getActivityAlien().push_back(score);
			}
			// region
			else
			{
				j -= _rules->getCountries().size();
				getRegistry().findValueByName<Region>(_rules->getRegions()[j])->getActivityAlien().push_back(score);
			}
		}
	}
	...
	void SaveConverter::loadDatLoc()
	{
		...
		for (size_t i = 0; i < ENTRIES; ++i)
		{
			...
			case TARGET_CRAFT:
				target = new Craft(_mod->getCraft(_rules->getCrafts()[0], true), 0, id);
				break;
			case TARGET_XBASE:
				target = new Base(_mod);
				break;
			case TARGET_ABASE: {
				AlienBase* abase = new AlienBase(_mod->getDeployment("STR_ALIEN_BASE_ASSAULT", true), 0);
				abase->setId(id);
	...
	void SaveConverter::loadDatBase()
	{
		...
		std::vector<entt::entity> baseIds(bases.size());
		auto baseValues = bases | std::views::transform([](Base* base) { return *base; });
		getRegistry().raw().create(baseIds.begin(), baseIds.end());
		getRegistry().raw().insert(baseIds.begin(), baseIds.end(), baseValues);
	}
