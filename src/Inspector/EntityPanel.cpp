...
//
//	std::optional<SimpleRTTR::Type> _componentType;
//	GetComponentFunc _getComponentFunc;
//
//	EntityComponentData(entt::handle entityHandle, std::optional<SimpleRTTR::Type> componentType, GetComponentFunc getComponentFunc)
...
//{
//	Bind(wxEVT_SEARCHCTRL_SEARCH_BTN, &EntityPanel::onSearch, this, ID_SearchCtrl);
//	Bind(wxEVT_TREE_SEL_CHANGED, &EntityPanel::onEntitySelected, this, ID_EntityTree);
//	Bind(wxEVT_TIMER, &EntityPanel::onTimer, this, ID_RefreshTimer);
//
//	setupUI();
...
//{
//	Unbind(wxEVT_SEARCHCTRL_SEARCH_BTN, &EntityPanel::onSearch, this, ID_SearchCtrl);
//	Unbind(wxEVT_TREE_SEL_CHANGED, &EntityPanel::onEntitySelected, this, ID_EntityTree);
//	Unbind(wxEVT_TIMER, &EntityPanel::onTimer, this, ID_RefreshTimer);
//}
//
//
...
//	if (nameComponent)
//	{
//		std::string entityName = data->_componentType.value().name();
//	}
//
...
//	_entityDetailGrid->Clear();
//
//	std::string componentName = data->_componentType.value().name();
//
//	_entityDetailGrid->Add("Component", [&componentName](wxWindow* parent) { return new wxStaticText(parent, wxID_ANY, componentName); });
//
//	if (data->_componentType.has_value())
//	{
//		// loop through the properties
//		for (const SimpleRTTR::Property& property : data->_componentType.value().properties())
//		{
...
//				{ return _typePropertyMapping.createProperty(parent, data->_entityHandle, data->_componentType.value(), property); });
...
//
//		_entityTree->SetItemData(entityItem, new EntityComponentData(entt::handle(_registry, entity), std::nullopt, nullptr));
//
...
//			{
//				std::optional<SimpleRTTR::Type> type;
//
//				// get the type information
//				std::string typeName = std::string(entities.type().name());
//				SimpleRTTR::TypeManager::iterator typeIterator = std::find_if(SimpleRTTR::types().begin(), SimpleRTTR::types().end(), [typeName](const SimpleRTTR::Type& type) { return type.name() == typeName; });
//				if (typeIterator != SimpleRTTR::types().end())
//				{
//					type = *typeIterator;
//				}
//				
...
//				if (type.has_value())
//				{
//					componentItem = _entityTree->AppendItem(entityItem, type.value().name());
//
...
//					if (type.value().meta().has(GetComponentFuncName))
//					{
//						const SimpleRTTR::Meta& getComponentMeta = type.value().meta().get(GetComponentFuncName);
...
//					else
//					{
//						_entityTree->SetItemData(componentItem, new EntityComponentData(entt::handle(_registry, entity), type, nullptr));
//					}
//				}
//				else
//				{
...
//					showMessage("Unable to get component data for component type \"" + data->_componentType.value().name() + "\"");
//				}
//			}
//			else
//			{
